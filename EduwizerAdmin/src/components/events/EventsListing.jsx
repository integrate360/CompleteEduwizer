...
import { addEvent, deleteEvent, getEventById, getEvents, updateEvent } from "../../Services/api";
import { getApiMessage, isSuccessValue } from "../../Services/responseUtils";
import EventFormModal from "./EventFormModal";
import { getEmptyEventForm } from "./eventFormUtils";
...
export default function EventsListing() {
  ...
  const [form, setForm] = useState(getEmptyEventForm());
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (!isSuccessValue(data?.success)) throw new Error(getApiMessage(data, "Failed to load events."));
      const rows = Array.isArray(data?.data) ? data.data : [];
      setItems(rows);
    } catch (e) {
      toast.error(e?.response?.data?.message || e?.message || "Error loading events.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = async (id) => {
    ...
    try {
      ...
      const data = resp?.data;
      if (!isSuccessValue(data?.success)) throw new Error(getApiMessage(data, "Failed to load event."));
  ...
  const openDetails = async (row) => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.message || e?.message || "Failed to load event details.");
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        const resp = await updateEvent({ ...payload, eventId: editingId });
        ...
        const resp = await addEvent(payload);
  ...
  const onConfirmDelete = async () => {
    ...
    if (!id) return;
    try {
      const resp = await deleteEvent({ eventId: id });
    ...
    <div className="w-full bg-white rounded-3xl border border-slate-200 p-4 md:p-8 shadow-sm flex flex-col relative">
      <div className="flex flex-col border-b border-slate-100 pb-5 mb-5 gap-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold text-brand-navy">Events</h2>
            <p className="text-xs text-slate-400 mt-1 font-light">Review, search, and manage events shown on the user website.</p>
          </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-96 gap-4">
          ...
          <p className="text-sm font-medium text-slate-500">Loading events from server...</p>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-between">
          <div className="overflow-x-auto scrollbar-none border border-slate-100 rounded-2xl">
            <table className="min-w-full divide-y divide-slate-100 bg-white">
              ...
              <tr>
                <th className="px-6 py-4.5 text-left text-xs font-bold text-brand-navy uppercase tracking-wider">Event</th>
      ...
      <ConfirmModal
        isOpen={confirmOpen}
        title="Delete Event"
        message={deleteTarget.title ? `Delete “${deleteTarget.title}”? This cannot be undone.` : "Delete this event? This cannot be undone."}
        confirmText="Delete"
        cancelText="Cancel"
        onCancel={() => {
          if (saving) return;
      ...
      <Modal
        open={detailsOpen}
        title="Event Details"
        description="View full event information."
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <div className="mt-1 text-sm font-semibold text-brand-navy">{selectedEvent.title || "-"}</div>
              </div>
              <div className="md:col-span-2">
                <div className="text-xs font-semibold text-slate-600">Content</div>
                <div className="mt-1 text-sm text-slate-700 whitespace-pre-wrap">{selectedEvent.description || selectedEvent.data || "-"}</div>
              </div>
              {selectedEvent.image ? (
                <div className="md:col-span-2">
                  <div className="text-xs font-semibold text-slate-600">Image</div>
                  <img src={selectedEvent.image} alt="" className="mt-2 w-full max-h-72 object-contain rounded-2xl border border-slate-200 bg-white" />
                </div>
              ) : null}
            </div>
