...
const emptyForm = { name: "", title: "", rating: 5, date: "", description: "" };
...
export default function TestimonialsListing() {
  ...
  const fetchPage = async ({ page, limit } = pagination) => {
    setLoading(true);
    try {
      const resp = await getTestimonials({ page, limit, search });
      const data = resp?.data;
      if (!isSuccessValue(data?.success)) throw new Error(getApiMessage(data, "Failed to load testimonials."));
      ...
      toast.error(e?.response?.data?.message || e?.message || "Error loading testimonials.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = async (id) => {
    ...
    setSaving(true);
    try {
      const resp = await getTestimonialById(id);
  ...
  const onSave = async () => {
    if (!form.name || !form.description || !form.date) return toast.error("Name, date, and description are required.");
    const ratingNum = Number(form.rating);
    if (Number.isNaN(ratingNum) || ratingNum < 1 || ratingNum > 5) return toast.error("Rating must be between 1 and 5.");

    setSaving(true);
    try {
      const payload = { ...form, rating: ratingNum };
      if (editingId) {
        const resp = await updateTestimonial({ ...payload, testimonialId: editingId });
        ...
        const resp = await addTestimonial(payload);
  ...
  const executeDelete = async () => {
    ...
    try {
      const resp = await deleteTestimonial({ testimonialId: itemToDelete });
  ...
  return (
    <div className="w-full bg-white rounded-3xl border border-slate-200 p-4 md:p-8 shadow-sm flex flex-col relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-6 mb-6 gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-brand-navy">Testimonials</h2>
          <p className="text-xs text-slate-400 mt-1 font-light">Manage testimonials shown on the Eduwizer dashboard.</p>
        </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="text-left px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Rating</th>
              ...
              {rows.map((row) => (
                <tr key={row._id} className="hover:bg-slate-50/40 transition">
                  <td className="px-6 py-4">
                    <div className="text-sm font-semibold text-brand-navy">{row.name}</div>
                    {row.title ? <div className="text-xs text-slate-400 font-light">{row.title}</div> : null}
                  </td>
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center bg-brand-gold/15 border border-brand-gold/25 text-amber-800 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                      {row.rating}/5
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-xs text-slate-600 font-semibold">{row.date}</span>
                  </td>
      ...
      {/* View Modal */}
      <Modal open={viewOpen} title="Testimonial Details" onClose={() => setViewOpen(false)}>
        <div className="space-y-4">
          <div className="text-sm font-semibold text-brand-navy">{viewData?.name}</div>
          <div className="text-xs text-slate-600">{viewData?.title}</div>
          <div className="text-xs text-slate-500">Rating: {viewData?.rating}/5</div>
          <div className="text-xs text-slate-500">Date: {viewData?.date}</div>
          <div className="text-xs text-slate-500">{viewData?.description}</div>
          <MediaField valueUrl={viewData?.url || ""} valueFileType={viewData?.fileType || ""} label="Testimonial Image" disabled />
        </div>
      </Modal>

      <Modal open={modalOpen} title={editingId ? "Edit Testimonial" : "Add Testimonial"} onClose={() => setModalOpen(false)}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-semibold text-slate-600">Name</label>
            <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} className="mt-1 h-10 px-4 text-sm border border-slate-200 rounded-xl outline-none focus:border-brand-gold focus:ring-4 focus:ring-brand-gold/15 transition w-full bg-slate-50 focus:bg-white text-brand-navy" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" min={1} max={5} value={form.rating} onChange={(e) => setForm((f) => ({ ...f, rating: e.target.value }))} className="mt-1 h-10 px-4 text-sm border border-slate-200 rounded-xl outline-none focus:border-brand-gold focus:ring-4 focus:ring-brand-gold/15 transition w-full bg-slate-50 focus:bg-white text-brand-navy" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-600">Date</label>
            <input value={form.date} onChange={(e) => setForm((f) => ({ ...f, date: e.target.value }))} placeholder="e.g. 2026-05-25" className="mt-1 h-10 px-4 text-sm border border-slate-200 rounded-xl outline-none focus:border-brand-gold focus:ring-4 focus:ring-brand-gold/15 transition w-full bg-slate-50 focus:bg-white text-brand-navy" />
          </div>
          <div className="md:col-span-2">
            <label className="text-xs font-semibold text-slate-600">Description</label>
            <textarea value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} rows={5} className="mt-1 px-4 py-3 text-sm border border-slate-200 rounded-xl outline-none focus:border-brand-gold focus:ring-4 focus:ring-brand-gold/15 transition w-full bg-slate-50 focus:bg-white text-brand-navy" />
          </div>
        </div>
