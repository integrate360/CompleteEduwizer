...
export default function FeaturedListing() {
  ...
  const subtitle = "Manage homepage featured media items (image/video/YouTube embed).";

  const fetchPage = async ({ page, limit } = pagination) => {
    setLoading(true);
    try {
      const resp = await getFeaturedLists({ page, limit, type: selectedType });
      const data = resp?.data;
      if (!isSuccessValue(data?.success)) throw new Error(getApiMessage(data, "Failed to load featured listings."));
      ...
      toast.error(e?.response?.data?.message || e?.message || "Error loading featured listings.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = async (id) => {
    ...
    setSaving(true);
    try {
      const resp = await getFeaturedListById(id);
      const row = normalizeSingleRecordResponse(resp?.data);
      if (!row) throw new Error("Record not found.");
      setForm({ url: row.url || "", fileType: row.fileType || "" });
    } catch (e) {
  ...
  const onSave = async () => {
    if (!form.url) return toast.error("Please provide a media URL (or upload a file).");
    setSaving(true);
    try {
      if (editingId) {
        const resp = await updateFeaturedList({ ...form, featuredListId: editingId });
        ...
        const resp = await addFeaturedList(form);
  ...
  const executeDelete = async () => {
    ...
    try {
      const resp = await deleteFeaturedList({ featuredListId: itemToDelete });
    ...
    <div className="w-full bg-white rounded-3xl border border-slate-200 p-4 md:p-8 shadow-sm flex flex-col relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full md:w-auto">
          <div className="relative w-full sm:w-auto">
            <select
              ...
              className="h-10 pl-4 pr-10 text-sm border border-slate-200 rounded-xl outline-none focus:border-brand-gold focus:ring-4 focus:ring-brand-gold/15 transition-all w-full sm:w-48 text-brand-navy bg-slate-50 focus:bg-white cursor-pointer appearance-none font-medium"
            >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-72 gap-4">
          ...
          <p className="text-sm font-medium text-slate-500">Loading featured items…</p>
        </div>
      ) : rows.length === 0 ? (
        <div className="border border-dashed border-slate-200 rounded-2xl h-72 flex flex-col items-center justify-center bg-slate-50/30 gap-3 text-slate-400">
          <p className="text-sm font-semibold text-slate-700">No featured items found</p>
          <p className="text-xs text-slate-400 font-light">Add the first one to get started.</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-2xl border border-slate-200">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                <th className="text-left px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Media</th>
                ...
                <tr key={row._id} className="hover:bg-slate-50/40 transition">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-14 h-10 rounded-xl border border-slate-200 bg-slate-50 overflow-hidden flex items-center justify-center">
                        {(row.fileType || "").includes("image") ? (
                          <img src={row.url} alt="" className="w-full h-full object-cover" />
                        ) : (row.fileType || "").includes("video") ? (
                          <div className="text-[10px] font-bold text-slate-500">VIDEO</div>
                        ) : (row.fileType || "").includes("youtube") || row.fileType === "youtube" ? (
                          <div className="text-[10px] font-bold text-slate-500">YT</div>
                        ) : (
                          <div className="text-[10px] font-bold text-slate-500">LINK</div>
                        )}
                      </div>
                      {/* URL link removed */}
                    </div>
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center bg-slate-50 border border-slate-200 text-slate-600 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider">
      ...
      <Modal
        open={modalOpen}
        title={editingId ? "Edit Featured Item" : "Add Featured Item"}
        description="Changes reflect on the Eduwizer dashboard."
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              ...
              onClick={onSave}
              disabled={saving}
              className="h-10 px-4 bg-brand-navy hover:bg-brand-purple text-white font-bold rounded-xl text-sm transition disabled:opacity-60"
            >
              {saving ? "Saving…" : "Save"}
            </button>
          </div>
