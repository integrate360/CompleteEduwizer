...
export default function AdministratorsListing() {
  ...
  const subtitle = "Manage teachers/lecturers/administrators shown on the Eduwizer dashboard.";

  const fetchPage = async ({ page, limit } = pagination) => {
    setLoading(true);
    try {
      const resp = await getTeachers({ page, limit, search });
      const data = resp?.data;
      if (!isSuccessValue(data?.success)) throw new Error(getApiMessage(data, "Failed to load administrators."));
      ...
      toast.error(e?.response?.data?.message || e?.message || "Error loading administrators.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSave = async () => {
    if (!form.name || !form.position || !form.country || !form.location) {
      return toast.error("Please fill name, position, country, and location.");
    }
    ...
    try {
      if (editingId) {
        const resp = await updateTeacher({ ...form, teacherId: editingId });
        ...
        const resp = await addTeacher(form);
  ...
  const executeDelete = async () => {
    ...
    try {
      const resp = await deleteTeacher({ teacherId: itemToDelete });
    ...
    <div className="w-full bg-white rounded-3xl border border-slate-200 p-4 md:p-8 shadow-sm flex flex-col relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-6 mb-6 gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 font-light">{subtitle}</p>
        </div>
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full md:w-auto">
          ...
          <button
            onClick={openAdd}
            className="h-10 px-4 bg-brand-gold hover:bg-btn-gold-hover text-brand-navy font-bold rounded-xl text-sm transition-all shadow-md shadow-brand-gold/15 cursor-pointer whitespace-nowrap"
          >
            Add New
          </button>
          <button
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="text-left px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">LinkedIn</th>
                ...
                <tr key={row._id} className="hover:bg-slate-50/40 transition">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl border border-slate-200 bg-slate-50 overflow-hidden">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-slate-400 font-light truncate">{row.position}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-xs text-slate-600 font-semibold">{row.location}</div>
                    <div className="text-[11px] text-slate-400 font-light">{row.country}</div>
                  </td>
                  <td className="px-6 py-4">
                    {row.linkedIn ? (
                      <a className="text-xs font-semibold text-brand-navy underline break-all" href={row.linkedIn} target="_blank" rel="noreferrer">
                        Open
                      </a>
                    ) : (
      ...
      {/* View Modal */}
      <Modal open={viewOpen} title="Administrator Details" onClose={() => setViewOpen(false)}>
        <div className="space-y-4">
          ...
          <div className="text-xs text-slate-500">LinkedIn: {viewData?.linkedIn ? (
      ...
      <Modal open={modalOpen} title={editingId ? "Edit Administrator" : "Add Administrator"} onClose={() => setModalOpen(false)}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input value={form.position} onChange={(e) => setForm((f) => ({ ...f, position: e.target.value }))} className="mt-1 h-10 px-4 text-sm border border-slate-200 rounded-xl outline-none focus:border-brand-gold focus:ring-4 focus:ring-brand-gold/15 transition w-full bg-slate-50 focus:bg-white text-brand-navy" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-600">Country</label>
            <input value={form.country} onChange={(e) => setForm((f) => ({ ...f, country: e.target.value }))} className="mt-1 h-10 px-4 text-sm border border-slate-200 rounded-xl outline-none focus:border-brand-gold focus:ring-4 focus:ring-brand-gold/15 transition w-full bg-slate-50 focus:bg-white text-brand-navy" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-600">Location</label>
            <input value={form.location} onChange={(e) => setForm((f) => ({ ...f, location: e.target.value }))} className="mt-1 h-10 px-4 text-sm border border-slate-200 rounded-xl outline-none focus:border-brand-gold focus:ring-4 focus:ring-brand-gold/15 transition w-full bg-slate-50 focus:bg-white text-brand-navy" />
          </div>
          <div className="md:col-span-2">
            <label className="text-xs font-semibold text-slate-600">LinkedIn (optional)</label>
            <input value={form.linkedIn} onChange={(e) => setForm((f) => ({ ...f, linkedIn: e.target.value }))} placeholder="https://linkedin.com/in/..." className="mt-1 h-10 px-4 text-sm border border-slate-200 rounded-xl outline-none focus:border-brand-gold focus:ring-4 focus:ring-brand-gold/15 transition w-full bg-slate-50 focus:bg-white text-brand-navy" />
          </div>
          <div className="md:col-span-2">
            <MediaField
              valueUrl={form.url}
