...
import { getPackageByCounseller, postcreatePayment, checkPaymentStatusAPI } from "../../Services/api";
...
const CounsellerPackageScreen = () => {
  ...
  // Fetch packages for counselor
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const response = await getPackageByCounseller();
                ...
                {packages.map((pkg) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "8px",
                      backgroundColor: selectedPackage === pkg ? "#e6f7ff" : "#f9f9f9",
                      cursor: "pointer",
