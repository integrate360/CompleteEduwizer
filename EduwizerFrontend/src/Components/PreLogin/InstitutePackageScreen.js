...
import { getPackageByInstitute, postcreatePayment, checkPaymentStatusAPI } from "../../Services/api";
...
const InstitutePackageScreen = () => {
  ...
  useEffect(() => {
    getPackageByInstitute()
      ...
      .catch((error) => {
        console.error("Error fetching packages for institute:", error);
        setLoading(false);
      });
  }, []);
  ...
  const handlePackageSelect = (pkg) => {
    ...
    setError(""); // Reset error message when a package is selected
  };
  ...
  const handleScreenshotUpload = (e) => {
    ...
    if (file) {
      setScreenshot(file);
      setError(""); // Reset error message on file selection
    }
  };
